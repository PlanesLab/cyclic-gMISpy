...
from .cobraTools import buildReactionFromBranch, createSparseMatrix, Name, parse, re, calculateMCS, parseGPRToModel, simplifyGMatrix, relatedRows, mergeIsforms, initiateLogger
...
def calculateGMIS(cobraModel, regulatory_dataframe, num_layers=2, **kwargs):
    ...
    startTime = time.time()
    gObject = calculateRegNetGMatrix(cobraModel, regulatory_dict, num_layers, solver, maxKOLength=maxKOLength, path=path, onlyGenes=onlyGenes)
    endTime = time.time()
...
def calculateRegNetGMatrix(model, regulatory_dict, num_layers=2, solver='gurobi', **kwargs):    
    ...
    for key, value in tqdm(gprDict.items()):
        ...
        if len(expandedRulesWithCycles['sources']) == 0:
            calculateTraditionalMCS(gDict, value['gpr'], value['reactions'], maxKOLength)
        else:
            numInteractions += len(expandedRulesWithCycles['sources'])
            boolForm = boolTransformer(expandedRulesWithCycles, value)
            with suppress_output():
                gmcs = bonesisCalculateCutSets(boolForm, maxKOLength = maxKOLength)
            if len(gmcs) == 1:
                # This means the only solution found is blocking the original gpr
                continue
            if len(gmcs) > maxMarkers:
                maxMarkers = len(gmcs)
    ...
    print(f'The max number of markers is {maxMarkers}')
    print(f'The number of interactions is {numInteractions}')
    nameGenesFile = f'Genes.txt'
    filename =  path + '/' + nameGenesFile
...
def calculateTraditionalMCS(gDict, gpr, reactions, maxKOLength=5):
    ...
    # Only or
        ...
        solutionDict = calculateMCS(model, MAX_LENGTH_MCS=maxKOLength, MAX_MCS=1e6, rxnSubset=list(treeGenes))
        for _, data in solutionDict.items():
            # Rename the genes as they are KOs
